  ...
  for key, value in payload.items():
    if key == "temperature":
      if value > settings.temperature_threshold:
        await send_threshold_alert(message="Temperature threshold exceeded: {value}")
        logger.info(f"Temperature threshold exceeded: {value}")
    elif key == "humidity":
      if value > settings.humidity_threshold:
        await send_threshold_alert(message="Humidity threshold exceeded: {value}")
      ...
      await send_threshold_alert(message="Conductivity threshold exceeded: {value}")
